...
**Stats to load:**
- Events: Count Query on `events` table with user_id filter
- Artists: Count Query on `artists` table with user_id filter
- Venues: Count Query on `venues` table with user_id filter
...
**List Items:**
- Links to Detail Pages (/events/{id})
...
- "New Artist" → /artists/new
- "New Venue" → /venues/new
